...
static asmlinkage long fh_sys_getdents(struct pt_regs *regs)
{
	...
	pr_info("getdents() before\n");

	ret = real_sys_getdents(regs);

	pr_info("getdents() after: %ld\n", ret);
...
static asmlinkage long fh_sys_getdents64(struct pt_regs *regs)
{
	...
	kernel_filename = duplicate_filename((void *)((struct linux_dirent64 *)regs->si)->d_name);

	pr_info("getdents64() before; id: %ld, dirent: %s, count: %ld\n", regs->di, kernel_filename, regs->dx);

	ret = real_sys_getdents64(regs);

	kfree(kernel_filename);

	pr_info("getdents64() after: %ld\n", ret);
...
static struct ftrace_hook demo_hooks[] = {
	...
	HOOK("sys_getdents", fh_sys_getdents, &real_sys_getdents),
	HOOK("sys_getdents64", fh_sys_getdents64, &real_sys_getdents64),
	//HOOK("sys_old_readdir", fh_sys_execve, &real_sys_execve),
};
